...
pr=prcomp(t(ms[rowVars(ms)>1,]),scale. = TRUE,center=TRUE)
#pr=prcomp(t(ms130[rowVars(ms130)>0,]),scale. = TRUE,center=TRUE)
variances<-pr$sdev^2
pdf("PCA_MsBuccalSaliva130kVar.pdf", width=12, height=12)
plot(x=pr$x[,1],y=pr$x[,2],main="Ms by Batch",col=alpha(c("red","orange","blue","green","purple"),0.5)[batches],pch=20,cex=1,xlab=sprintf("PC1 (%3.2f%%)",100*(variances[1]/sum(variances))),ylab=sprintf("PC2 (%3.2f%%)",100*(variances[2]/sum(variances))))
#legend("bottomleft",legend =c("Epi","Fat","IC"),fill=c("red","green","blue") )
legend("bottomright",legend = c("DS","Smokers","Mothers","Saliva"),fill = alpha(c("red","orange","blue","green","purple"),0.5))
dev.off()
pdf("PCA_MsBuccalSaliva130kVarByCellType.pdf", width=12, height=12)
plot(x=pr$x[,1],y=pr$x[,2],main="Ms by Cell Type",col=celltypecol,pch=20,cex=1,xlab=sprintf("PC1 (%3.2f%%)",100*(variances[1]/sum(variances))),ylab=sprintf("PC2 (%3.2f%%)",100*(variances[2]/sum(variances))))
#legend("bottomleft",legend =c("Epi","Fat","IC"),fill=c("red","green","blue") )
#legend("bottomright",legend = c("DS","Smokers","Mothers","Saliva"),fill = alpha(c("red","orange","blue","green","purple"),0.5))
dev.off()
