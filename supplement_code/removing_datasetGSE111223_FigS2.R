...
#REMOVING DATASET GSE111223 
commonProbes=intersect(colnames(df50586),intersect(colnames(df94876),(colnames(df137688))))
#Sanity checks
...
df_comb=rbind(df50586[,colnames(df50586)%in%commonProbes],df94876[,colnames(df94876)%in%commonProbes],df137688[,colnames(df137688)%in%commonProbes])
#Combine the ages and status effects as well to correspond to each of the samples
ages_comb=c(as.numeric(GSE50586$GSE50586_series_matrix.txt.gz$`age:ch1`),as.numeric(ages),as.numeric(meta137688$Age))
status_comb=c(rep("DS",10),rep("Healthy",10),smoker,rep("Healthy",length(meta137688$Age)))
...
batches=c(rep(1,20),rep(2,120),rep(3,250))
...
legend("bottomright",legend = c("DS","Smokers","Mothers"),fill = alpha(c("red","orange","blue","green"),0.5))
dev.off()
...
dfmat130=df_mat[rownames(df_mat)%in% common[,1],batches<4]
...
legend("bottomright",legend = c("DS","Smokers","Mothers"),fill = alpha(c("red","orange","blue","green"),0.5))
dev.off()
write.table(model130$finalModel$coefficients,quote=FALSE,sep="\t",file = "130CommonModelCoefficients.tsv")
