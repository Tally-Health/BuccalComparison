...
#PCA health
#healthcol=rep("green",dim(ms)[2])
#healthcol[status_comb=="Moist Snuff User"]<-"orange"
#healthcol[status_comb=="DS"]<-"blue"
#healthcol[status_comb=="PD"]<-"purple"
#healthcol[status_comb=="Cigarette Smoker"]<-"red"
...
dfmat_elovl=df_mat[rownames(df_mat)%in% elovl,batches<5]
#dfmat130=combated[rownames(combated)%in% common[,1],]
mse=getM(dfmat_elovl)
#ms130=ms[row.names(ms) %in% common[,1],]
dfe=as.data.frame(t(mse[,status_comb=="Healthy"]))
dfe$Age=as.numeric(ages_comb[status_comb=="Healthy"])
dfe$Epi=out2[status_comb=="Healthy",1]
dfe$Fib=out2[status_comb=="Healthy",2]
dfe$B=out2[status_comb=="Healthy",3]
dfe$NK=out2[status_comb=="Healthy",4]
dfe$CD4T=out2[status_comb=="Healthy",5]
dfe$CD8T=out2[status_comb=="Healthy",6]
dfe$Mono=out2[status_comb=="Healthy",7]
dfe$Neutro=out2[status_comb=="Healthy",8]
#impute missing values
impute=preProcess(dfe, method = c("medianImpute"))
dfe=predict(impute,dfe)
set.seed(13)
modelE <- train(Age~.,data=dfe,method="lm",trControl=control, preProcess=c("center","scale"))
pdf("BuccalSalivaHealthyELOVLlm.pdf")
plot(modelE$pred$obs,modelE$pred$pred,pch=18,main="ELOVL2 lm",xlab="Age",ylab="Pred. Age",xlim=c(20,100),ylim=c(20,100),col=alpha(c("red","orange","blue","green"),0.5)[batches[status_comb=="Healthy"][modelE$pred$rowIndex]])
#plot(modelE$pred$obs,modelE$pred$pred,pch=18,main="ELOVL2 lm",xlab="Age",ylab="Pred. Age",xlim=c(20,100),ylim=c(20,100),col=celltypecol[status_comb=="Healthy"][modelE$pred$rowIndex])
abline(fit <- lm(pred ~ obs, data=modelE$pred), col='red')
abline(c(1,1), col='gray')
legend("topleft", bty="n", legend=paste("R2 is", format(modelE$results$Rsquared, digits=4)," RMSE is ",format(modelE$results$RMSE, digits=4)," MAE is ",format(modelE$results$MAE, digits=4)))
legend("bottomright",legend = c("DS","Smokers","Mothers","Saliva"),fill = alpha(c("red","orange","blue","green","purple"),0.5))
dev.off()

write.table(modelE$finalModel$coefficients,quote=FALSE,sep="\t",file = "ELOLV2ModelCoefficients.tsv")
